...
interface RecruiterSubscriptionDashboardProps {
  companyId: string;
  subscription: CompanySubscription;
}

export function RecruiterSubscriptionDashboard({ 
  companyId, 
  subscription 
}: RecruiterSubscriptionDashboardProps) {
  ...
  const loadData = async () => {
    try {
      ...
      const [payments, invoicesList, summary, details] = await Promise.all([
        paymentHistoryApi.getCompanyPaymentHistory(companyId),
        paymentHistoryApi.getCompanyInvoices(companyId),
  ...
  const handleCancelSubscription = async () => {
    try {
      setCancelling(true);
      await subscriptionApi.cancelCompanySubscription(companyId);
      await loadData();
