...
export const SocialShareIcons: React.FC<SocialShareIconsProps> = ({
  resume,
  className = '',
  size = 'md',
  variant = 'icons'
  ...
  const quickShare = async (platform: string) => {
    ...
    const coreSkills = resume.skills?.slice(0, 4).map(skill => 
    ...
    switch (platform) {
      case 'whatsapp':
        ...
        break;
      case 'linkedin':
        window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(shareTitle)}&summary=${encodeURIComponent(shareDescription)}`, '_blank');
        break;
      case 'facebook':
  ...
  // Default icons variant with better styling
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      <button
        onClick={generateShareLink}
        disabled={isGenerating}
        className="p-2 rounded-full hover:bg-blue-50 text-gray-600 hover:text-blue-600 transition-all duration-200 hover:scale-110 disabled:opacity-50"
