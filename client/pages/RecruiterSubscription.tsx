...
import { RecruiterUsageTracker } from '@/components/subscription/RecruiterUsageTracker';
import { RecruiterSubscriptionDashboard } from '@/components/subscription/RecruiterSubscriptionDashboard';
import { subscriptionApi } from '@/services/subscriptionApi';
import type { CompanySubscription, SubscriptionUsage } from '@shared/subscription';
...
export default function RecruiterSubscription() {
  ...
  const loadSubscriptionData = async () => {
    try {
      ...
      const subData = await subscriptionApi.getCompanySubscription(companyId.toString());
      setSubscription(subData);
      
      if (subData) {
        const usageData = await subscriptionApi.getCompanyUsage(companyId.toString());
        setUsage(usageData);
      }
    } catch (err: any) {
  ...
  const handleSelectPlan = async (planId: string, billingCycle: 'monthly' | 'yearly') => {
    try {
      ...
      if (result.paymentUrl) {
        ...
        // Subscription created without payment (e.g., free trial)
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-700 mt-1">
                    Select a subscription plan to unlock premium features and start hiring top talent.
                  </p>
                </div>
              </div>
            </CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    ...
                    <Button variant="outline" onClick={() => navigate('/recruiter/subscription/plans')}>
                      Change Plan
                    </Button>
                  </div>

                  <RecruiterUsageTracker subscription={subscription} usage={usage} />
                </div>
              </CardContent>
            </Card>
          </TabsContent>
